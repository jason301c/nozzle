...
21. **Saga recovery:** prove handler-version retention; shard-local effect-receipt atomicity; duplicate and contradictory receipt rollback; stale route, schema, and partition-fence rejection; exact-primary observation on the original binding; the complete confirmed, unknown, retryable non-application, exhausted non-application, terminal failure, and compensation classification matrix across both ledgers; exact replay evidence; atomic rollback and lost-response recovery at terminal receipt consumption; every forward and compensation unknown-outcome combination; cancellation; timeout; and process or deployment restart.
