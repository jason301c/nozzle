...
Control schema version 2 preserves the version 1 rule that lease rows are never deleted, so fencing tokens can never reset. The immutable version 1 installation identity remains in `nozzle_control_meta`; schema publication is an append-only `nozzle_control_schema_versions` history whose maximum row is authoritative. The version 2 installer rejects a future version before installing version 2 objects, installs and exactly verifies each replacement saga guard before removing the version 1 trigger name, and publishes version 2 last. Protocol-2 classification uses separately verified identity, structural binding, current-action, observation-causality, and compensation-causality guards. Until schema version 2 is published, the mapping guard forbids protocol 2, the mapper emits protocol 1, and a static audit rejects any preexisting protocol-2 row; after publication the same mapper emits protocol 2. This immutable publication boundary replaces mutable current-state audits, so a migration-window attempt may settle or lose its lease without making an interrupted installer unrecoverable. The bounded guard decomposition is required to remain below D1's SQLite expression-depth limit while preserving the full predicate. The installer is idempotent at every interruption boundary, remains non-destructive under statement-atomic lock-step and skewed two-connection installer interleavings, and preserves the original installation timestamp. Lease acquisition, renewal, release, and authorization use D1 server time, the shared pure lease reference model, and exact optimistic compare-and-swap predicates. A takeover advances the token by exactly one and is permitted only after release or authoritative expiry; shard-local triggers independently reject token rollback, token jumps, unfenced identity changes, active-lease takeover, and deletion. Compare-and-swap retries are bounded at 16 before the operation enters an actionable intervention state.

Control schema version 3 adds immutable irreversible-authorization classification without changing the six application statements in a coupled saga transition. The exact version 1 and version 2 installation artifacts remain frozen at SHA-256 `6537f6ef35f24d62831e751779758fc6eb04c7938850b28abfa23f4c6335afaa` and `962e6e62174590d666ec75a3c111c2a92298fbf2996e3dadd5f9b6a7739b4909`. Before version 3 publication, a full authorization body is rejected and an old worker's checksum-only irreversible dispatch is trigger-classified as protocol 1. The installer conservatively backfills earlier dispatches as protocol 1, exactly verifies the receipt table and every decomposed guard, audits all classified history, and publishes version 3 last. After publication, an irreversible dispatch requires the exact 14-field authorization body, a 64 KiB maximum, canonical operation/plan/step/input binding, the current lease identity and fence, a nonfuture D1 seal time, and a trigger-derived protocol-2 receipt at the transition timestamp. A retry additionally requires a full retained authorization body in its predecessor; a checksum-only legacy retry remains quarantined. The body and its top-level checksum must be preserved on every non-dispatch transition, while a proven-not-applied retry may replace them only with a fully sealed authorization and leaves the earlier body in immutable history. Provider and saga effect-attempt inserts after publication require a protocol-2 receipt matching the current operation-step projection. Protocol-1 history remains visible for diagnosis and historical verification but cannot by itself authorize a post-version-3 effect or terminal settlement. SQLite supplies structural, transactional classification rather than cryptographic rehashing or terminal authority; complete-history checksum verification, internal capability minting, and exclusive API/schema authority remain release-blocking.
...
Every coupled saga mutation commits one operation-transition receipt, one operation-effect receipt, the saga projection insert or exact-version update, the operation-step update, the derived operation-status update, and one audit event in a single Control D1 batch under the same active lease fence. Receipt inserts are plain conflict-failing inserts: an exact concurrent winner is verified after rollback, while a contradictory conflict can never leave a partial projection. The typed coordinator derives transition, effect, audit, and evidence identities internally; a runner cannot supply an arbitrary after-record or receipt identity. Canonical operation step IDs are `saga:init`, `saga:settle`, `saga:termination`, and `saga:<phase>:<descriptor-step-id>`. The coordinator rejects a transition whose step, state, attempt, projection version, or receipt chain does not match both ledgers. Action receipt acceptance additionally requires both the matching canonical operation-step state and the matching saga action state, so an operation transition alone cannot authorize dispatch. Saga action operation steps are declared independently in the generic plan because a failed forward must not make compensation ineligible through ordinary success-only dependency edges; the sealed serial saga state machine remains the eligibility oracle.

An irreversible generic or saga dispatch stores the complete sealed authorization in both the current canonical operation-step record and the immutable transition's `to_record_json`; a checksum alone is not durable authority. Sealing and loading own one exact plain snapshot, enforce the 64 KiB body limit before trust, verify the domain-separated checksum, and bind operation, plan, step input, lease identity, acquisition, and fence. The version 3 transition trigger derives the separate protocol receipt inside the first transition statement, so the coordinator still has exactly six application statements and complete rollback. Later success, failure, crash recovery, reconciliation, classification, and terminal-model reads retain and revalidate the exact body. A status replay may omit authorization; if it supplies one, every field must equal the trusted retained body. A retry after proven non-application requires a trusted retained predecessor body and a full sealed current authorization, which may be newly sealed; checksum-only legacy state cannot cross that boundary. Exact concurrent replays validate the winning immutable transition and its checksum-verified audit event rather than requiring two contenders to have sampled the same D1 timestamp. These receipts are necessary evidence for the complete-history verifier but do not themselves mint the internal terminal-settlement capability.

Initialization replay is receipt-first. It reconstructs the exact historical operation predecessor and version-zero saga from the immutable initialization transition, effect, and audit event, then returns the checksum-verified current descendant even after later action progress, coordinator restart, or a newer-fence takeover. Once the initialization transition is visible, the coordinator reloads both current projections before replay verification, so a concurrent winner cannot be rejected because of stale operation or saga reads taken before that atomic commit.

The canonical cancellation and timeout admission path is a typed coordinator operation. Within one exact saga and operation, a stable request ID is bound to the cause and caller request checksum; reusing that scoped ID with different intent is a contradiction. The coordinator moves the pristine conditional `saga:termination` step directly from pending to a succeeded one-attempt record in the same six-statement Control D1 batch that records the termination effect and saga projection. The coordinator uses this direct atomic outcome only for its canonical reversible coordinator-owned `opaque` step; the primitive rejects `provider_receipt` and `saga_receipt` steps. Timeout eligibility uses authoritative D1 server time and is rechecked against the immutable deadline and absent prior cause inside the committing batch. Cancellation uses the same absent-cause guard. The first successful compare-and-swap therefore fixes the cause; a concurrent loser verifies and returns that exact winner rather than overwriting it.

Termination replay is receipt-first and historical. It reconstructs the operation predecessor and prior saga version from the exact transition, audit event, unique coupled effect, and version chain; semantically reruns both state machines at the persisted D1 request time; and verifies the immutable current descendant and termination operation step. The still-active original lease proof may replay the result, while an active strictly newer fence on the same lease may consume it after restart or takeover. Missing, duplicated, stale, malformed, or contradictory evidence requires intervention. A saga already terminal, or already terminating because of an automatic forward failure, remains unchanged rather than manufacturing a cancellation or timeout receipt.

This evidence proves the typed coordinator path, not exclusive saga authority. Release remains blocked until the legacy mutable saga projection API and generic or raw schema paths cannot create a split termination outside that atomic batch.

The coordinator, not the runner, consumes terminal action receipts and classifies both ledgers atomically. `confirmed` succeeds the saga action with the output checksum and succeeds the generic action step with the complete receipt-outcome checksum. `unknown` makes both ledgers unknown. Proven `not_applied` remains retryable in both ledgers while the sealed saga retry budget remains; on the final allowed attempt it becomes a terminal saga failure while the generic action step succeeds with the classified receipt-outcome checksum. A terminal `failed` receipt follows the same generic classification-success rule while producing saga failure or compensation intervention. Generic success in these terminal cases means that the receipt-classification operation completed, never that the business effect succeeded; the saga projection alone controls compensation and terminal business status. Every replay revalidates the exact immutable operation transition, canonical operation-step JSON, derived operation status, saga effect, saga record checksum and JSON, state version, receipt outcome, and original lease fence. Semantically identical operation-step records always use one canonical JSON encoding so property insertion order can never block a retry or create a false conflict.

The coordinator also consumes terminal observation receipts only under the strictly newer active fence and exact causal effect attempt sealed by the acceptance receipt. A confirmed observation atomically marks the saga effect applied and the generic step reconciled; observed non-application remains retryable in both ledgers until the saga's sealed action budget is exhausted, when the generic step records successful terminal classification while the saga alone records forward failure or compensation intervention; an indeterminate observation requires intervention in both ledgers. The observation transition, operation effect, saga projection, operation step and status, and audit event use the same conflict-rollback Control D1 batch and exact replay verification as direct effect classification. Observation-purpose outcomes are limited to confirmed, not-applied, or indeterminate in both the receipt API and Control schema.

The saga plan compiler snapshots every caller-controlled top-level input exactly once before its first asynchronous boundary, owns the step-input checksum map, and uses that single snapshot for validation, compilation, handler binding, and provenance. It emits required `saga:init` and `saga:settle`, conditional `saga:termination`, and every possible conditional forward and compensation action before execution. Action steps use `saga_receipt`, have no ordinary dependency edges, and bind the descriptor artifact, schema, static input, lease, and recovery contract. An irreversible forward is an irreversible checkpoint and has no compensation step. A loaded generic plan becomes saga-specific only after complete canonical recompilation produces the same sealed plan checksum for that exact saga. Terminal orchestration freezes the terminal saga version and checksum: branch cleanup and settlement are operation-only transitions and never manufacture another saga checkpoint or effect. It marks every unchosen conditional path `not_required` using that checksum-verified terminal saga projection as decision evidence. An abandoned retryable action becomes generic classification success only from its exact durable non-application evidence: the original terminal receipt for a direct classification, the recovery evidence for proven crash-before-dispatch absence, or the causal observation evidence for observed absence. `saga:settle` advances only after the complete projection/effect chain verifies and every conditional branch is classified; saga success maps to operation success, saga intervention maps to operation intervention, and every other terminal saga status maps to operation failure.

Control-plane saga attempt receipts are append-only dispatch and observation evidence; they do not replace the shard-local receipt that a D1 application mutation commits atomically with its data change. Attempt inputs, evidence, outputs, and errors are canonical JSON capped at one MiB, with domain-separated payload, acceptance, and outcome checksums. The version 2 acceptance receipt binds the exact descriptor action key, saga and operation identity, phase, purpose, idempotency key, payload, lease fence, and tagged causal-attempt identity. A first forward effect has no cause; an observation names the exact unknown effect attempt it investigates; and compensation names the exact confirmed forward effect it counteracts. Control schema checks and triggers independently enforce the canonical generic step identity, sealed `saga_receipt` plan, column-to-record agreement, and causal receipt/output checksums against checksum-verified saga and operation projections. An accepted effect without a terminal outcome remains evidence of possible dispatch and is recovered as unknown with its acceptance checksum recorded as the error evidence in both ledgers; absence of an acceptance receipt is the only Control-plane proof that dispatch did not occur. Recovery rechecks that accepted-without-outcome or absent-attempt condition inside the same six-statement D1 batch that updates both projections, so a racing receipt cannot be misclassified. A terminal receipt discovered after takeover is settled, not downgraded to unknown: its original attempt binding is revalidated and the immutable classification transition records the active consumer fence, which may be the exact receipt fence or a strictly newer fence on the same lease.

`nozzle_saga_action_attempt_protocols` immutably distinguishes protocol 1 rows accepted before or during the upgrade from attempts accepted after verified schema version 2 publication. The version 2 mapper is installed only after the strict attempt, outcome, mapping, and mapping-immutability guards have been exactly verified. It emits protocol 1 while schema version 2 remains unpublished, conservatively backfills every unmapped legacy or migration-window attempt as protocol 1, and emits protocol 2 only after the final publication statement. The installer rejects every prepublication protocol-2 row and statically audits exact attempt identity and classification time; it never re-audits mutable saga, operation, outcome, or lease state. Existing legacy-invalid terminal rows remain visible for intervention and are never rewritten; every new outcome, including one for a legacy accepted attempt, obeys the version 2 purpose/outcome matrix. Completing a legacy accepted observation reruns the full checksum-verified saga, operation, and causal-receipt binding. Before any first projection mutation, terminal protocol 1 effect and observation receipts and accepted-without-outcome recovery receipts rerun that same exact binding under the receipt's original fence; immutable historical replay is verified first so later legitimate progress does not make an already-consumed receipt appear stale. An old weak, contradictory, or stale acceptance therefore remains visible but cannot mutate either projection.
...
68. The generic operation transition is part of the same conflict-rollback Control D1 batch as its saga effect, saga projection, operation step and status, and audit event; public runners use a typed coordinator that derives the coupled records and immutable identities rather than composing projection stores.
69. The typed coordinator alone consumes terminal saga action receipts. Confirmed, unknown, retryable non-application, exhausted non-application, and terminal failure have one explicit cross-ledger mapping; generic terminal classification success is distinct from business success, and exact replay is bound to both canonical projections and their immutable transition/effect receipts.
70. The typed coordinator alone consumes terminal saga observation receipts. Acceptance is causally bound to the exact unknown effect under a newer fence; applied, retryable or exhausted non-application, and indeterminate outcomes have one atomic cross-ledger mapping and exact replay proof.
71. Control schema upgrades publish through an immutable append-only version history, reject future versions before mutation, and never destructively swap the installation identity. Saga protocol upgrades classify legacy rows explicitly, verify replacement trigger definitions before publication, and conservatively treat every migration-window attempt as legacy rather than claiming stronger proof than was recorded.
72. D1 safety predicates that exceed the platform's SQLite expression-depth limit are decomposed into independently installed and definition-verified guards, with bounded static provenance audits before publication. Publication requires every component; no combined predicate may be weakened merely to fit the engine limit.
73. Saga initialization replay is historical and receipt-first: it reconstructs version zero from immutable coupled evidence, accepts only a checksum-verified current descendant, and reloads projections after observing the receipt so a concurrent winner cannot create a false contradiction.
74. The canonical saga cancellation and timeout admission path is one typed, six-statement atomic Control transition. Authoritative D1 time and an in-batch first-cause/deadline guard choose one immutable winner; scoped stable request identity detects changed intent; historical receipt-first replay supports exact retries, restart, and newer-fence takeover without allowing receipt-owned steps to use the internal direct-outcome primitive. The package root exposes only stable saga protocol identities rather than the legacy mutable saga projection store. The generic operation store remains responsible for creating the canonical operation envelope and for the prerequisite `saga:init` acceptance; after creation, it rejects every other step mutation of a reserved `saga:` operation. Action, termination, and settlement acceptance; every generic outcome; crash recovery; and conditional branch classification fail before persistence. Initialization acceptance owns one stable request and lease-proof snapshot before validation or asynchronous work, so switching accessors cannot cross that exception. The coordinator rejects fresh initialization under a non-reserved operation type; only receipt-first replay or continuation of an already materialized legacy saga can use that compatibility path. This is fail-closed API hardening, not exclusive authority. Operation creation, the remaining initialization acceptance, internal relative store access, unrestricted Control D1 binding, raw schema mutation, terminal-history verifier, and terminal capability remain release-blocking.
75. Terminal saga branch cleanup and settlement never advance the terminal saga projection. The immutable terminal version and checksum are the decision evidence for operation-only branch and settlement transitions. Pure outcome mapping and branch-decision functions check the local model but grant no settlement authority: caller-supplied records, versions, checksums, or authorization maps are not proof. Production settlement requires an internal, unforgeable capability minted only after complete immutable-history verification of both ledgers; the current structural projection loader explicitly does not mint it. The model requires a checksum-verified descriptor body, owns one stable view of both projections and its model evidence, and validates the exact saga step membership and envelopes, termination tuple, retry exhaustion, serial forward order, reverse compensation order, and compensation causality as well as each canonical action record. It retains direct receipt, crash-absence recovery, and observation-based non-application as distinct decision kinds. Exact replay cannot substitute evidence between those kinds. Atomic Control persistence, the capability-minting complete-history verifier, and exclusive schema/API authority remain release-blocking proofs.
76. Every checksum-bearing generic operation boundary owns one exact plain snapshot before validation or its first asynchronous boundary. Plan sealing and loading, complete operation-record loading, irreversible authorization sealing and loading, audit append and loading, and full-chain audit verification never reread caller-owned state after hashing begins; persisted loaders reconstruct only exact checksum-covered fields. Checksum-bearing arrays are dense and undecorated, nested irreversible lease records and proofs have exact schemas, an audit append verifies its exact predecessor, and chain verification rejects malformed event or array envelopes before accepting hashes. Proxy, accessor, unknown-field, sparse-array, and post-capture mutation failures fail closed under stable public errors rather than creating a trusted mixed-time object.
77. An irreversible dispatch persists the complete checksum-verified authorization body as canonical operation state and immutable transition history; the checksum is an index, not a substitute for the body. The body is capped at 64 KiB and bound to the sealed operation, plan, step input, lease identity, acquisition, and fence. Schema version 3 classifies migration-window checksum-only dispatches as protocol 1, publishes only after exact guard and history audits, and derives protocol 2 from full bodies after publication. Protocol 1 cannot authorize a post-publication provider or saga effect. A checksum-only retryable predecessor is quarantined; a valid retry must retain a trusted full predecessor body and persist the full current authorization, leaving both attempts reconstructible. The trigger-derived receipt preserves the exact four-statement generic and six-statement coupled application batches. It is structural transactional evidence, not the cryptographic complete-history verifier or the unforgeable terminal capability required by Decision 75.
78. Control schema version 4 freezes the complete version 3 install artifact and installs a row-safe terminal saga-attempt representation without activating an incompatible writer format. Once explicitly activated, each new terminal outcome stores its evidence and output or error body in two separate immutable companion rows, each capped by UTF-8 byte length at one MiB. The existing outcome row becomes a compact header containing exact canonical storage references and the payload checksums; all three rows commit in one conflict-failing D1 batch, and response-loss recovery trusts only an exact checksum-verified reload. Zero companion rows always denotes a legacy inline outcome, while any partial, extra, contradictory, noncanonical, or checksum-invalid companion set requires intervention. Schema publication alone leaves the activation ledger empty, so version 3 and version 4 workers continue to write and read the exact inline format during rollout; a one-MiB guard over the frozen reader's joined input, evidence, and terminal value prevents either worker from creating an unsafe compatibility result row. The mandatory order is additive version 4 schema publication, version 4 reader deployment, compatible-reader convergence proof, then activation. Activation is one immutable, persistent protocol-1 row carrying the checksum of that verified deployment barrier. It is rejected before version 4 publication, audited empty at publication, and has no production minting API until the user-owned controller can enumerate and prove every active reader compatible. Tests may insert a fictional activation only to qualify the prepared format. This is deliberate fail-closed availability during rollout: a combined inline receipt above that compatibility bound cannot complete until the barrier is proven. The activation and companion tables, exact definition and immutability guards, and empty prepublication audits install before version 4 publishes last; rerunning an installed schema remains safe. Point loading reads the acceptance identity, outcome header, and at most three companion rows separately, never a wide input-plus-outcome join. The activated local maximum-size representation is safely smaller than D1's documented 2,000,000-byte row limit without weakening the one-MiB per-payload cap. Verified production activation and exact remote D1 boundary qualification remain release-blocking.
79. Complete terminal-history verification starts from one internal, read-only paging boundary rather than an unbounded projection query. It captures a terminal saga head together with the current operation identity, transition count and high-water pair, dense saga-effect count and head, saga-attempt count and high-water pair, and a pinned environment audit head, then rereads every mutable or terminal head before returning the version-1 anchor. A changed terminal saga or effect head requires intervention; a concurrently advancing operation or attempt tail requires restart from a fresh anchor. Audit events page by dense environment sequence; operation transitions and their optional irreversible-authorization receipt page by audit-sequence and transition-ID keyset; saga effects page by the exact dense `0..terminalVersion` chain; and attempt acceptance identities page by acceptance-time and attempt-ID keyset without joining input to outcome payloads. Text keysets explicitly use D1's `BINARY` UTF-8 byte ordering, including local validation, rather than JavaScript's different UTF-16 code-unit ordering. Every page query uses `LIMIT 3` to return at most two rows, never uses `OFFSET`, owns one dense exact result snapshot, validates explicit columns and known saga effect kinds, rejects a premature end before its anchored high-water mark, and enforces cumulative UTF-8 fetch and return budgets. The module is absent from the package root and exposes no mutation or settlement capability. This is the locally qualified bounded history source, not complete proof: exhaustive cross-stream set reconciliation, supporting-index and maximum-history performance qualification, durable Workflow continuation, final anchor reconciliation after the fold, private capability minting, and atomic terminal-tail persistence remain release-blocking.
80. The terminal-history audit fold owns each page and the anchor before checksum work begins, admits only one in-flight page, and applies a page only after every row and its terminal metadata verify, so an overlapping or rejected page cannot stale-write or leave partial fold state. It reconstructs every event through the canonical audit-event loader, requires the exact canonical JSON body, and verifies the dense sequence, hash link, nondecreasing server time, pinned environment, and anchored head across arbitrary page boundaries. Within that complete environment chain, the target operation must have exactly one input-bound unfenced `operation.created` event before any of its transition events; every other target event must identify a fenced step, and their count must equal the anchored operation-transition count. A domain-separated rolling SHA-256 fold over their ordered sequence and already verified event hashes produces constant-size evidence for the operation-transition fold without retaining the environment history. The folder and anchor loader remain internal, expose no D1 write, and mint no terminal capability. This proves the audit side only: exhaustive cross-stream reconciliation; final anchor reconciliation; capability minting; and terminal-tail persistence remain release-blocking.
81. The terminal-history operation fold point-loads the immutable operation plan from its exact Control D1 row, owns one canonical body bounded to 2,000,000 UTF-8 bytes, and verifies the anchored operation identity and sealed checksum before using it. It owns each transition page before asynchronous work, admits only one in-flight page, and commits a page only after every transition verifies, so overlap or rejection cannot stale-write or partially advance the projection. Stable transition identities use unambiguous length-framed inputs, and receipt-to-transition binding is exact. Protocol-2 history reconstructs every generic operation state through the canonical operation reducers; protocol-1 history uses explicit reducer-equivalent compatibility reconstruction. Both require exact keyset order, predecessor and successor records, derived status, transition event, canonical JSON, audit sequence, and rolling audit-transition checksum. Saga initialization is bound only to `saga:init`; conditional reversible opaque termination is bound only to `saga:termination`; saga action labels are bound only to their sealed `saga_receipt` forward or compensation step; and generic step receipt labels cannot relabel a saga action to bypass the coordinator. Exact fence progression, predecessor authorization preservation, receipt relationships, transition payload links, and crash-derived provider-absence evidence are verified. Protocol-1 checksum-only history remains readable for compatibility but has no authority and a checksum-only retry predecessor is quarantined. Protocol 2 verifies the complete irreversible-authorization body, derived receipt, preservation, and a seal time no later than the verified audit event. The folder is internal and read-only, exposes no provider or settlement capability, and mints no terminal capability. This proves the generic operation-transition stream locally, not the complete two-ledger terminal history: deterministic saga action-begin and recovery evidence across those streams, exhaustive cross-stream reconciliation, final anchor reconciliation, private capability minting, and atomic terminal-tail persistence remain release-blocking.
82. The terminal-history saga-effect fold owns each dense effect page and the anchor before checksum work, admits only one in-flight page, and commits no partial state from an overlapping or rejected page. It loads every canonical saga record through the checksum-verifying core loader; independently verifies the domain-separated record checksum, deterministic coordinator effect identity, canonical action-step and transition identity, immutable saga intent, exact state version, nondecreasing D1 time, one lease key, nondecreasing fence, stable acquisition at an equal fence, and a strictly newer recovery fence; and reconstructs creation, begin, success, every direct failure classification, accepted-unknown or not-dispatched labeled crash recovery, cancellation, timeout, applied or absent or indeterminate observation, and reverse compensation through the production saga reducers. Retry and termination decision time is derived from checksum-covered state while the later effect-row time is an upper bound, so verification neither invents time nor rejects an authorized transition merely because persistence crossed a clock boundary. A domain-separated rolling fold provides constant-size ordered evidence, and completion requires the exact anchored effect count, head, record checksum, terminal status, and final saga projection. The folder remains internal and read-only and mints no terminal capability. This proves the saga projection stream locally, not the complete two-ledger terminal history: deterministic receipt and recovery evidence across all streams; exhaustive cross-stream reconciliation; final anchor reconciliation; private capability minting; and atomic terminal-tail persistence remain release-blocking.
83. The terminal-history saga-attempt fold owns each identity page before asynchronous verification, admits only one in-flight page, and applies no partial state from an overlapping or rejected page. Each paged identity is decoded independently, then its outcome header and at most three valid companion payload rows are point-loaded separately and passed through the canonical receipt codec. Acceptance, input, evidence, terminal output or error, protocol classification, canonical JSON, and outcome checksums must all verify; an accepted receipt cannot retain orphaned terminal payloads. The fold requires exact operation, saga, action-step, lease, nonregressing later-time fences, stable acquisition identity at an equal fence, action-handler, idempotency, keyset, count, and anchored-head bindings. Once the complete stream is present, every observation must identify the exact accepted-or-unknown effect in the same action slot under a strictly newer fence and derived observation key, while every compensation must identify a confirmed forward effect with output evidence; no dependent acceptance may predate its cause's terminal outcome. Causality closes only after the stream because equal-millisecond keyset ordering may place a dependent identity before its cause. An exact empty stream is valid, and a domain-separated rolling checksum plus immutable compact summaries binds every ordered accepted or terminal receipt without retaining payload bodies. The folder remains internal and read-only and mints no terminal capability. This proves the receipt stream locally, not the complete two-ledger terminal history: deterministic action-begin, terminal, recovery, and observation evidence across all streams; exhaustive cross-stream reconciliation; final anchor reconciliation; private capability minting; and atomic terminal-tail persistence remain release-blocking.
84. Canonical saga-plan history verification reuses the one production compiler rather than maintaining a second plan model. The reader point-loads the immutable descriptor ID, version, checksum, and canonical body from the anchored saga row; the checksum-verifying core descriptor loader must reproduce every exact action artifact and policy reference within the D1 row bound. Only after the effect folder has reconstructed the complete terminal saga may it combine that persisted descriptor with the already checksum-verified operation plan. It owns the descriptor before asynchronous work, requires the descriptor and plan to match the effect folder's anchor, and recompiles the full required initialization and settlement steps, conditional termination, every forward action, and every eligible compensation from the reconstructed saga ID, saga input, per-step inputs, descriptor, capability snapshot, idempotency key, and plan lease. The existing compiler's exact sealed checksum must equal the anchored plan checksum. The resulting immutable compact proof records descriptor, saga, operation, type, and exact step counts but carries no handler registry, mutation method, or settlement capability. This proves descriptor and plan identity locally, not complete two-ledger history: deterministic action-begin, terminal, recovery, and observation evidence across all streams; exhaustive cross-stream reconciliation; final anchor reconciliation; private capability minting; and atomic terminal-tail persistence remain release-blocking.
85. Exhaustive saga-history reconciliation consumes only completed audit-bound operation, saga-effect, and saga-attempt folders together with the descriptor-bound canonical plan proof. It requires all component proofs to name the same operation and saga, then forms an exact bijection between every coupled saga audit transition and every saga effect by transition identity, event kind, operation step, evidence checksum, lease key, holder, acquisition, and fence. Every effect attempt must bind the descriptor's exact action artifact, stable saga idempotency key, canonical operation action slot, forward input checksum when applicable, and one deterministic action-begin effect under the acceptance fence. A terminal attempt consumes exactly one direct outcome effect with its outcome and output or error checksum; an accepted effect consumes exactly one newer-fence unknown-recovery effect with its acceptance checksum; an observation consumes exactly one deterministic reconciliation transition for its causal effect and terminal outcome; and a begin with no acceptance receipt consumes exactly one strictly newer-fence not-dispatched recovery carrying its independently reconstructed deterministic evidence. Forward and compensation effects and observations follow the same rules, and every non-begin action effect must be consumed exactly once, so missing, duplicate, relabeled, mixed, unresolved, or extra history requires intervention. The result is an immutable compact proof binding the component fold checksums and exact counts. The reconciler remains internal and read-only, exposes no D1 binding or handler registry, performs no mutation, and mints no terminal capability. Final post-fold anchor reconciliation, supporting-index and maximum-history performance qualification, durable Workflow continuation, private capability minting, and atomic terminal-tail persistence remain release-blocking.
86. Final saga-history reconciliation owns a fresh copy of the captured anchor before asynchronous work and runs exhaustive cross-stream reconciliation before any final database assertion. It binds the completed audit head and environment; operation input, plan, status, transition count, and transition high-water pair; saga descriptor, input, effect count and head, record checksum, state version, and terminal status; attempt count and high-water pair; and every compact reconciliation identity and count as one exact tuple. Only then does it invoke the production reader's unoverridden prototype implementation to point-read the current operation and terminal saga identity, transition summary, dense effect summary, attempt summary, and pinned audit row. A changed terminal saga, effect head, or pinned audit row requires intervention; an operation or attempt tail that advanced during the fold requires restart from a fresh anchor. The immutable final proof contains the exact anchor and reconciliation proof but remains internal evidence with no D1 mutation method and no settlement authority. Node tests inject an operation-head mutation during plan reconciliation and prove it is observed only by the final re-read; malformed anchors, fake readers, and folded heads from another anchor fail closed. Real workerd D1 evidence runs the same final boundary after all four paged folds. Supporting-index and maximum-history performance qualification, durable Workflow continuation, private capability minting, and atomic terminal-tail persistence remain release-blocking.
87. Terminal settlement authority is an opaque, frozen, fieldless in-process capability whose identity exists only in module-private weak maps. The final verifier registers its reconstructed operation and saga only after rerunning plan and cross-stream checks through the fold classes' unoverridden prototype methods, matching every head to the owned anchor, and completing the production reader's final database re-read. A plain object, structural cast, clone, serialized value, copied final proof, or proof from a failed or overridden verifier has no authority. Minting from the same live final proof is idempotent; process loss discards the capability and requires complete reverification. The mint derives terminal-model evidence exclusively from the registered history state, including the exact verified saga checksum and state version and every required irreversible authorization checksum retained in the reconstructed operation. The existing strict terminal model must accept that exact operation and saga before the capability can bind immutable branch decisions and the mapped settlement outcome. Capability loading is a separate internal runtime check for the future persistence boundary; neither minting nor loading is exported from the package root, and neither performs a D1 write. Node evidence covers every terminal outcome, direct and observed compensation, retry and recovery evidence, irreversible authorization, idempotent minting, cloning and fake-object rejection, and primitive rejection. Real workerd D1 evidence mints and validates both reversible and irreversible terminal histories. Atomic capability-gated terminal-tail persistence, supporting-index and maximum-history performance qualification, and durable Workflow continuation remain release-blocking.
88. Atomic saga terminal persistence is available only through the internal `D1SagaTerminalStore` and requires the live opaque capability from Decision 87. It captures the actor and an owned lease-proof snapshot once, applies every verified branch decision and the final settlement through the canonical core reducers, and emits deterministic immutable transition identities plus explicit not-required, direct-receipt, crash-absence, observed-absence, and settlement audit semantics. One `D1Database.batch()` contains exactly four statements regardless of plan width: a JSON1-driven bulk transition insert, a bulk step update, one operation-status update, and an ordered bulk audit append. The first statement rechecks the exact anchored operation identity, status, timestamp, transition count and high-water pair; every complete operation-step record; terminal saga identity and checksum; dense effect count and head; attempt count and high-water pair; pinned audit event; and active lease identity and fence inside the transaction. Complete step projections are split across at most ten separately bound JSON arrays below D1's 2,000,000-byte value limit and expanded through a `VALUES` table, while transition and audit arrays are independently bounded; the statement remains below the 100-parameter and 100,000-byte SQL limits. A batch error is non-authoritative: exact immutable receipts either prove that the complete tail committed, including one lease proof, a contiguous audit subchain, the unchanged saga/effect/attempt heads, the exact transition-count descendant, and the final operation projection, or require retry/intervention. A newer lease owner may recognize an exact prior commit without rewriting it, while a fresh proof over an already settled operation is a verified no-op. Node evidence covers all successful, failed, and intervention outcomes; all three terminal non-application evidence shapes; direct and observed compensation; stale anchors and leases; malformed capabilities, inputs, metadata, receipts, audit rows, heads, and projections; exact concurrent writers; lost responses; every four-statement rollback position; post-commit refolding; a 1,999,500-byte step-record boundary; and the maximum 256-step descriptor, 515-step operation plan, and 513-transition terminal tail. Real workerd D1 evidence executes the same four-statement path for reversible and protocol-2 irreversible sagas and refolds both completed histories. The store and capability remain absent from the package root. This discharges the local atomic terminal-tail persistence obligation referenced as outstanding in Decisions 79 through 87; remote D1 maximum-boundary qualification, supporting-index and maximum-history performance, durable Workflow continuation, and exclusive raw/schema authority remain release-blocking.

89. The installed Control package now has one audited root export and no store subpaths. The root omits the mutable saga-attempt and saga-coordinator stores as well as every complete-history and terminal mutation boundary, while retaining only pure saga protocol helpers, public types, and stable step identities. The public generic `D1OperationStore` rejects a plan before any D1 access when its operation type, any step identity, or any effect protocol marks it as saga work. Internal production code receives saga-creation authority only by constructing the store through a factory whose instances are registered in a module-private weak set; that factory is absent from the root export. Node evidence independently exercises operation-type, step-label, and `saga_receipt` rejection with zero operation rows, and proves the marked internal path remains live. Package-surface evidence also fixes the export map to the root alone and proves initialization and attempt mutation classes are absent there. This closes the local installed-package operation-creation and initialization API escape described in Decision 74. It does not make source-tree relative imports, an unrestricted raw Control D1 binding, or direct schema SQL safe; the user-owned controller must become the sole deployed binding and schema authority before exclusivity is complete.

90. Control schema version 5 freezes the complete 186-statement version 4 artifact at SHA-256 `23d1467bb7b49c7f07aa3253af7d15beeb115edae159c74718fc749fda81712b` and adds immutable reader-version attestations plus one immutable saga-outcome reader barrier. Publication installs and definition-verifies both tables, their insert/publication and update/delete immutability guards, and a new activation trigger before auditing empty prepublication history and publishing version 5 last. An earlier version-4 activation without its exact barrier blocks the upgrade. Normalized deployment evidence names a sealed expected script set and exactly one latest active deployment per script; each deployment contains one or two traffic-bearing versions whose integer basis-point weights sum to 10,000. Every active version has exactly one artifact-checksummed attestation whose Control-schema range includes version 5 and whose explicit saga-outcome reader-protocol range includes version 1. Inputs are captured once, bounded to 256 scripts and 512 active versions, canonically sorted, domain-separated and SHA-256 bound, then held behind a frozen fieldless weak-map capability; duplicate attestation digests fail before persistence. The internal `D1ReaderBarrierStore` consumes only that live capability. One four-statement D1 batch bulk-inserts exact immutable attestations, inserts the canonical barrier only after rejoining every attestation, inserts the activation only from that barrier, and deliberately violates the schema-version check to roll back unless the exact activation receipt exists. Exact reload verifies both checksums, canonical barrier JSON, every referenced attestation body and checksum, time order, and the activation link. Preflight and post-failure reconciliation reject contradictory, incomplete, checksum-aliasing, or temporally impossible partial state instead of returning an endless retry; schema constraints also reject missing claimed JSON fields and activation before barrier verification. Node evidence covers gradual and single-version deployments, order independence, opaque-capability rejection, caller mutation, malformed inventory and attestations, all four rollback positions, lost response, exact replay, concurrent and between-read-and-batch contradictory winners, malformed D1 rows and metadata, interrupted and concurrent schema installation, corrupt definitions, and prepublication data. Real workerd D1 evidence executes the same gradual-deployment activation and reload. The verifier and store remain absent from the package root. This discharges the normalized local persistence and activation mechanism required by Decision 78; trusted Cloudflare API enumeration, signed external-pipeline attestations, deploy/activation race qualification, exclusive controller binding ownership, and remote Cloudflare evidence remain release-blocking.

91. The Cloudflare adapter now has a read-only active Worker-deployment observation boundary matching the documented `GET /accounts/{account_id}/workers/scripts/{script_name}/deployments` envelope. It performs exactly one redirect-rejecting, bounded-body request for one explicitly named script and trusts only the documented first deployment as the latest deployment actively serving traffic. The decoder requires canonical lowercase UUID deployment and version identities, a valid UTC creation time, percentage strategy, and exactly one or two unique traffic-bearing versions; percentages must convert without rounding to integer basis points and total exactly 10,000. Successful observations are deeply frozen and retain request start/completion time, status, bounded body size and state, SHA-256 response checksum, `cf-ray` when present, and conservative rate-limit evidence. Empty inventory, malformed bodies, invalid UTF-8, over-limit or unreadable streams, transport failure, abort, retryable status, and provider rejection remain distinct inconclusive results. The account ID, token, transport, clock, script name, signal, and byte-limit boundaries fail closed; the captured token is used only in the Authorization header and is never returned, while optional author email and deployment annotations are deliberately ignored. `Workers Scripts Read` is the minimum intended token permission even though the current API also accepts broader alternatives. Node adversarial evidence and real workerd evidence exercise the boundary using fictional recorded response shapes only. This implements credential-bearing Cloudflare transport and normalized active-deployment enumeration; it does not prove signed attestation verification, exact multi-script convergence, deployment-freeze or post-activation enforcement, permission preflight, a controller composition boundary, remote-account qualification, or authority to activate a real deployment.

92. Each complete Decision 91 observation now also carries a frozen fieldless capability backed by module-private weak-map state binding the exact Cloudflare account ID, normalized active deployment, and response evidence; clones and structural fakes carry no observation authority. A reader-deployment verifier accepts only those live proofs for one configured account and exact expected script set. Every active version must have one canonical version-1 statement signed with standard Ed25519 by a key in the verifier's captured trust registry. The domain-framed signed body binds key ID, explicit audience, script and version IDs, artifact checksum, Control-schema range, saga-outcome reader-protocol range, issuance time, and exclusive expiry time. Public keys are canonical unpadded base64url raw 32-byte Ed25519 keys; signatures are canonical unpadded base64url 64-byte values. The registry is bounded to 64 unique keys, so adding a key permits overlap and removing it prevents new verification without mutating earlier evidence. Statements must be issued before the earliest active-deployment request, remain unexpired at verification, include Control schema 5 and saga-outcome payload protocol 1, and fit an explicitly configured validity limit no greater than 30 days. All successful active observations must finish no later than verification, be no older than the configured limit, and fit a configured observation window; both observation policies have a hard maximum of five minutes. Inputs are captured before asynchronous cryptography, bounded to 256 scripts and 512 active versions, sorted with D1-compatible UTF-8 byte order, and retained behind a second opaque capability. The public signing-byte encoder and verified-evidence reader expose deterministic integration surfaces without granting Control mutation authority. Node evidence covers real signatures, key import and verification failure, account and audience replay, key rotation boundaries, malformed or noncanonical encodings, expiry, future/stale/window violations, caller mutation, fake capabilities, every compatibility mismatch, and the exact 256-script/512-version boundary. Real workerd evidence generates, signs, imports, and verifies standard Ed25519 keys against an account-bound active-deployment proof. This proves local signature and convergence semantics only: the artifact checksum remains a trusted signed assertion rather than a fetched Worker-version `etag`; signed envelopes and trust decisions are not yet durably recorded in Control history; exact second-pass deployment reobservation, exclusive controller composition, post-activation deployment enforcement, permission preflight, remote account qualification, and real activation remain release-blocking.
93. Every traffic-bearing version in a signed reader convergence proof now requires a second frozen fieldless capability produced only by the Worker-version detail adapter. The adapter performs one redirect-rejecting, bounded-body `GET /accounts/{account_id}/workers/scripts/{script_name}/versions/{version_id}` request, requires the response identity to equal the requested canonical UUID, and normalizes the documented `resources.script.etag` only when it is an exact lowercase SHA-256 checksum. Missing optional resource, script, or etag metadata is explicitly inconclusive; malformed identity, envelope, resources, script metadata, or checksum is rejected. Transport, bounded-body, response-checksum, `cf-ray`, rate-limit, status, and timing evidence use the same credential-redacting observation boundary as deployment enumeration, while author identity and unrelated version metadata are ignored. The opaque artifact proof binds the account, script, version, etag, and exact response evidence; it cannot be cloned or structurally forged and its mint and state reader remain absent from the package root. Signed convergence requires exactly one same-account artifact proof for every active version, rejects duplicates and inactive versions, includes both deployment and artifact reads in the configured freshness and convergence window, and requires the signed artifact checksum to equal the live etag before accepting the signature. The retained frozen evidence now includes each canonical signature with issuance and expiry plus each artifact etag and response checksum, without granting Control activation authority. Node evidence covers the documented response, encoded paths and signals, missing optional metadata, malformed envelopes and etags, retry/rejection/transport/body failures, account replay, fake and duplicate proofs, inactive and incomplete sets, every malformed evidence field, signed/live checksum mismatch, freshness, and the 512-version bound. Real workerd evidence fetches two fictional gradual-deployment version details and verifies their exact etags with standard Ed25519 signatures. This closes the local live-artifact comparison gap in Decision 92, but does not durably persist signed trust evidence, reobserve deployments immediately around activation, validate token permissions, enforce compatibility after activation, prove exclusive controller ownership, qualify a remote account, or authorize real activation.
94. Control schema version 6 freezes the complete 208-statement version-5 artifact at SHA-256 `5850ffaecd12f80f7a74fd50f16119bf741932192119827260b098a03a816f18` and adds one immutable `nozzle_reader_barrier_verifications` row for saga-outcome reader protocol 1. The row foreign-keys the exact normalized reader-barrier checksum, stores a unique verification checksum, caps canonical signed evidence at one MiB, binds its protocol, barrier, and verification time through database JSON constraints, and requires account, audience, artifact, attestation, deployment, and expected-script fields. A separate insert guard requires the normalized barrier to exist with a verification time no later than the signed evidence. Publication definition-verifies the table and its unpublished-insert, barrier-binding, update, delete, and activation-evidence triggers before auditing both verification history and the activation ledger empty and publishing version 6 last. Any version-5 activation, even one structurally valid under the earlier unsigned local mechanism, blocks the upgrade for intervention because no later record can prove which signature and trust decision authorized it. Once the version-6 activation trigger exists, an activation also requires an exact signed-verification row whose barrier exists and whose verification time is no later than activation. Interrupted and two-installer upgrades remain idempotent; corrupt definitions, prepublication evidence, unsigned activation, malformed JSON structure, time or identity mismatch, mutation, and deletion fail closed. The legacy normalized barrier store is qualified only against the frozen version-5 artifact, while latest-schema saga payload tests use clearly fictional signed-evidence fixtures. This is additive durable schema and enforcement infrastructure only: no production store can yet derive or insert the canonical verification row from Decision 93, and no public activation API exists. Exact signed-evidence canonicalization and reload, atomic signed activation, activation-bracketing deployment reads, controller ownership, permission validation, post-activation enforcement, and remote qualification remain release-blocking.

95. The internal `D1SignedReaderBarrierStore` is the sole local production-format bridge from the live opaque Decision 93 capability to the immutable Decision 94 verification and activation rows. Reader attestations at this boundary must span both Control schema 5 and schema 6 so the old and new reader formats remain safe across publication and activation. The store owns one captured evidence snapshot, rejects noncanonical or over-deep JSON, unpaired UTF-16, unsafe numbers, sparse or over-wide containers, and independently caps both the signed source and stored verification body at one MiB. Domain-separated SHA-256 checksums bind the complete source evidence and the verification body. That body retains the exact account and audience, expected script inventory, active deployments, Worker-version artifacts, bounded Cloudflare response observations, each attestation's signature and recorded trusted public key, source observation interval and verification time, normalized reader-barrier checksum, and an authoritative Control D1 verification time no earlier than the source verification. The store rederives the normalized barrier from the same source before any mutation. One five-statement D1 batch bulk-inserts at most 512 exact normalized attestations, inserts their canonical barrier, inserts the exact signed verification, activates only from that verification using D1 server time, and deliberately violates the schema-version guard unless the entire exact tail exists. Reload owns and canonicalizes every row, verifies both domain checksums and all time and barrier bindings, rederives the normalized capability, and returns a frozen receipt only when the signed and normalized histories agree. A batch exception is non-authoritative: exact immutable reload distinguishes a committed response loss from retryable absence and contradictory partial or concurrent state. Node evidence covers source capture, every canonical and byte boundary, invalid digests, malformed persisted rows, all five rollback positions, lost response, exact replay, authoritative-time failure, partial completion, malformed metadata, and exact or contradictory races. Real workerd evidence generates an Ed25519 key, observes fictional deployment and artifact API responses, verifies the signed attestation, runs the same D1 batch, and reloads the receipt. The store, normalized capability state, and mutation boundary remain absent from the package root. This supersedes Decision 94's statement that no production store exists and locally discharges exact signed-evidence canonicalization, durable reload, and atomic signed activation. External signatures, artifact equality, observation freshness, and configured trust were proved before the opaque capability was minted; reload proves the exact immutable record rather than reacquiring external state. A second deployment-and-artifact observation bracketing activation, exclusive user-owned controller composition and raw-binding authority, permission preflight, post-activation compatibility enforcement, remote D1 qualification, and any real-resource activation remain release-blocking.

96. New signed reader activation now requires an opaque two-observation stability capability rather than a single Decision 93 capability. The first and second capabilities must come from the existing account-bound signed verifier; the second observation must start no earlier than completion of the first verification, and the complete interval from the first API request through the second verification must fit an explicit stability window between one millisecond and five minutes. Their exact semantic identity must match across account, audience, expected scripts, deployment creation and identity, traffic weights, every active version and live artifact checksum, and every signed attestation field including public key, signature, issuance, and expiry. API response timestamps and checksums deliberately differ and are evidence, not deployment identity. A frozen fieldless weak-map capability retains both immutable observations; clones, serialization, structural fakes, a reused single observation, overlap, reversed order, expiry, or any semantic change have no authority. The private Control store accepts only that capability, persists the full second observation, the full first-source checksum, the first deployment and artifact response evidence, a domain-separated stable-identity checksum, both time intervals, and the configured window in the same immutable verification row, and rejects a D1 verification time outside that original window. The activation statement and rollback sentinel independently recheck the D1 activation timestamp against the same window, so expiry between preflight and the transaction rolls the entire batch back. A pair therefore cannot be cached for later activation. Reload verifies the compact first observation inventory against the second identity, every successful response envelope, both time orders, the stable-identity checksum, the complete verification checksum, and the normalized barrier. Exact committed Decision 95 single-observation rows remain reloadable. An unactivated verification row, whether single- or double-observed, requires intervention: the five-statement batch is atomic, and completing an externally introduced partial would activate without durably replacing its stale evidence. The production method can no longer create a new single-observation row. Node evidence covers exact and changed pairs, every policy and ordering failure, fake capabilities, stale Control time before and inside the transaction, malformed compact evidence, exact legacy reload, rejection of partial rows, the 256-script and 512-version boundary, all transactional failures, and a verification body no larger than one MiB. Real workerd evidence performs two distinct fictional deployment and artifact API rounds, verifies both with Ed25519, and commits the fresh pair through D1. This locally discharges the missing second-pass observation and activation-freshness mechanism from Decisions 92 through 95. It does not prevent a deployment initiated after the second read: exclusive user-owned deployment and Control authority, post-activation enforcement, permission validation, remote qualification, and real-resource activation remain release-blocking.

97. The installed Control package now exposes exactly one additional audited subpath, `@nozzle/control/controller`, containing only the high-level reader-deployment controller factory; the root remains unchanged and neither surface exports the signed store or any other private mutation class. The factory captures the expected reader inventory, signatures, trust registry, account, token, policy, clock, transport, digest, and supplied transactional Control binding before asynchronous work. Activation performs two complete account-bound deployment-and-artifact rounds, verifies both against the same captured signed inventory, mints the Decision 96 stability capability, commits through the private store, and then performs a third complete observation before returning. That immediate post-commit round must have the exact activated semantic identity; any changed deployment, weight, version, artifact, signature, audience, account, or expected script fails closed while the immutable receipt remains available for diagnosis. The same controller exposes an explicit read-only compatibility assertion for scheduled use, and the private store can compare a fresh signed observation with either a Decision 96 stable activation or an exact legacy Decision 95 activation without exposing stored evidence. API reads run in chunks of at most six simultaneous requests, matching the documented Worker connection bound. Before any request, the caller must provide an account-capability snapshot for external subrequests: activation conservatively reserves nine reads per expected script for three rounds with two active versions, and one compatibility assertion reserves three; values are bounded by Cloudflare's documented configurable paid maximum of ten million. Retryable status, transport failure, and abort remain resumable, while rejected or malformed responses and drift require intervention; no result or error contains the token. Node evidence covers activation, post-commit drift, continued compatibility, legacy continuity, absence, abort, retry and rejection classification, malformed configuration, byte and subrequest bounds, root/subpath authority, and token redaction. Real workerd evidence executes the controller composition, Ed25519 verification, three-round activation, D1 transaction, and a subsequent compatibility assertion. This closes the local controller composition and immediate post-activation observation gaps from Decision 96. It does not prove a deployed Worker is the only script with the raw Control binding or management secret, prevent an out-of-band deployment between checks, durably schedule compatibility checks, gate other operations on current health, rotate an activated reader set, inspect token policy, or qualify remote behavior; deployment-manifest exclusivity, permission preflight, durable continuous enforcement, authorized reader rotation, and remote evidence remain release-blocking.

For precision, the boundary fixture's checksum value is 1,999,500 UTF-8 bytes and its complete canonical operation-step record is 1,999,674 bytes, below D1's 2,000,000-byte limit; “1,999,500-byte step-record boundary” elsewhere in this checkpoint refers to that fixture. The fresh maximum-width persistence path prepares ten total D1 queries, including the four batched mutations, with every SQL statement below 100,000 bytes and no statement exceeding 100 bound parameters.
...
7. **Lease, provider, and irreversible-authorization fencing:** prove an expired controller cannot commit a protected D1 transition; prove external effects remain attributable, idempotent or unknown, and destructive calls require the complete sealed authorization body. Prove the version 2-to-3 mixed-worker window, protocol-1 backfill and quarantine, exact protocol-2 body/plan/fence binding, trigger-derived receipt atomicity, checksum-only retry quarantine, authorization retention through every state and retry, guarded provider and saga attempt acceptance, interruption recovery, response-loss replay, contradictory concurrency, and rollback at every generic and coupled batch statement.
...
21. **Saga recovery:** prove handler-version retention; shard-local effect-receipt atomicity; duplicate and contradictory receipt rollback; stale route, schema, and partition-fence rejection; exact-primary observation on the original binding; the complete confirmed, unknown, retryable non-application, exhausted non-application, terminal failure, and compensation classification matrix across both ledgers; causal newer-fence observation acceptance and atomic observation-receipt consumption for applied, non-applied, and indeterminate outcomes; accepted-without-outcome recovery bound to the same acceptance checksum in both ledgers; protocol 1 receipt quarantine and safe revalidation during the version 2 upgrade; historical initialization replay after later progress, restart, takeover, and a concurrent winning commit; atomic cancellation and timeout admission with authoritative D1 time, the in-batch deadline and first-cause guards, scoped stable request-intent collision rejection, every six-statement rollback position, cancellation-timeout interleavings, lost-response recovery, historical replay after action and compensation progress, restart, and newer-fence takeover, and rejection of missing, duplicated, or contradictory termination history; terminal cleanup across the maximum sealed plan width with immutable saga version/checksum evidence, deterministic unchosen-branch classification, distinct direct-receipt, crash-absence, and observed-absence evidence, exact classification replay, irreversible authorization preservation, and successful, failed, and intervention settlement; exact replay evidence; atomic rollback at every coupled batch statement and lost-response recovery for every terminal observation outcome; every forward and compensation unknown-outcome combination; cancellation; timeout; and process or deployment restart.
...
The atomic saga-termination path was revalidated on 2026-07-11 against Cloudflare's current guarantee that `D1Database.batch()` is a transaction whose complete sequence aborts or rolls back when a statement fails, Cloudflare's stated SQLite SQL semantics, and SQLite's definition of `unixepoch('subsec')` as the current fractional Unix time. Local workerd evidence exercises both the rollback-capable batch and time expression. This is not remote Cloudflare qualification, which remains release-blocking.

The terminal-orchestration model was locally revalidated on 2026-07-12 with exact safety coverage over single-snapshot saga-plan compilation, exact saga-specific plan recompilation, terminal-status and termination-tuple consistency, exact saga step envelopes, retry exhaustion and cross-action causal order, canonical cross-ledger branch classification, the three non-application evidence shapes and replay, irreversible authorization, malformed model inputs, and the sealed 256-step boundary. Package-surface and persistence tests also prove that the mutable saga projection store is absent from the package root and that generic action, termination, settlement, outcome, recovery, and branch-classification calls against reserved `saga:` operations fail before any `nozzle_%` table changes. Initialization acceptance captures every caller field once, owns cloned and frozen proof and step-checksum snapshots, rejects proxies or uncloneable input before persistence, and cannot be redirected to another step. Fresh initialization reconstructs the exact persisted version-1 invocation, matches its saga ID, input checksum, and descriptor-ordered step checksums, builds from those persisted values, and canonically recompiles the exact saga operation plan before any write; malformed, missing, extra, or mismatched invocation and plan evidence leaves every `nozzle_%` table unchanged. The coordinator rejects a newly created non-reserved saga-shaped operation, while receipt-first restart replay accepts an already materialized legacy initialization with its pre-envelope `{sagaId}` input and noncanonical plan without another write. The typed path remains live, and raw projection divergence is still detected. The internal saga-attempt codec owns exact independent identity, outcome-header, and bounded companion-row snapshots, accepts only explicit immutable columns, verifies canonical payloads and every acceptance and outcome checksum, rejects contradictory terminal value columns and malformed companion sets, covers every receipt state, and remains absent from the package root so a future paged history verifier can reuse it without expanding public authority. Caller-supplied model evidence and individually decoded rows are deliberately non-authoritative. Under a test-only fictional reader-barrier activation, schema version 4 stores the maximum one-MiB evidence and one-MiB terminal value in distinct rows, rejects a one-byte overflow including multibyte UTF-8 boundaries, atomically rolls back each failed three-statement position, recovers a lost commit response, resolves exact and contradictory races, and exercises the companion representation and reload through real workerd D1. The schema independently rejects and rolls back a three-row sidecar batch while activation is absent. A checksum-locked copy of the complete version 3 joined reader codec executes its exact query and decoder against version 4 inline outcomes at the one-MiB input-plus-outcome compatibility boundary; one additional byte is rejected. That frozen reader also rejects an activated reference header, which is why activation cannot precede the deployment barrier. The internal read-only history source now captures and rechecks stable terminal anchors, executes its audit, transition-plus-authorization, dense effect, and split attempt-identity keysets against SQLite and real workerd D1, returns no more than two rows from a `LIMIT 3` query, rejects gaps, forks, malformed rows, unknown effect kinds, premature tails, changing anchors, and byte-budget overflow, and remains absent from the public package surface. Its streaming audit fold owns every page before asynchronous checksum work, rejects overlapping page folds, applies no partial state from a rejected page, verifies each canonical event body and the complete pinned hash/time chain, rejects missing, duplicate, reordered, unfenced, or input-contradictory target-operation events, and produces the same constant-size ordered transition-event checksum across page boundaries in Node and workerd. The immutable operation plan is point-loaded from its exact D1 row and checksum-verified before use. The operation-transition fold then reconstructs every protocol-2 page through the canonical reducers and every protocol-1 page through explicit reducer-equivalent compatibility logic, verifies exact cursor, record, status, event, plan-label, fence, authorization, receipt, payload, provider-absence, and protocol-compatibility semantics, and reconciles the same rolling audit-transition checksum. Node adversarial and model tests cover page overlap and rejection, forged deltas and counters, protocol-1 retry quarantine, protocol-2 preservation and seal time, saga-label bypasses, fence contradictions, crash payloads, saga recovery classifications, and exact replay. Real workerd D1 evidence exercises the immutable plan load, paged audit chain, queried transition history, and final reconstructed projection. The saga-effect fold reconstructs the complete dense saga projection through the production reducers, verifies canonical records, record and effect checksums, deterministic transition, begin, and not-dispatched recovery evidence, exact retry and termination time, lease and recovery-fence progression, every direct, recovered, observed, cancellation, timeout, and compensation effect kind, and its exact terminal anchor without partial page state. Node adversarial evidence covers malformed pages, semantic relabeling, forged identity and evidence, invalid time, stale fences, overlap, rejection recovery, and every effect kind; real workerd D1 evidence pages and folds a canonical three-effect terminal saga. The saga-attempt fold independently point-loads and verifies every accepted, inline-terminal, or companion-terminal receipt; binds operation, saga, canonical action step, lease, fence acquisition, handler, idempotency, ordering, count, and anchored head; closes exact observation and compensation causality after the complete stream; and retains immutable compact summaries with a rolling checksum rather than payload bodies. Node adversarial evidence covers empty and paged streams, every outcome state, both protocols, inline and companion payloads, accepted receipts, malformed pages, orphan payloads, ordering and anchor contradictions, changed bindings, stale fences, missing or incompatible causes, overlap, and rejected closure. Real workerd D1 evidence folds an unknown effect and its newer-fence confirmed observation from split outcome and companion rows. The immutable descriptor body is now point-loaded separately and checksum-verified through the core loader. Only a completed effect folder can combine that descriptor with its reconstructed saga and the anchored operation plan; the production saga-plan compiler must reproduce the exact plan checksum and step set before returning a compact read-only proof. Adversarial Node evidence rejects missing, malformed, oversized, noncanonical, checksum-contradictory, wrong-saga, wrong-plan, untrusted, and uncapturable inputs. Real workerd D1 evidence loads the split descriptor and plan rows and recompiles the canonical five-step plan against the folded terminal saga. The cross-stream reconciler requires exact transition-to-effect bijection and lease evidence, descriptor-bound attempt identity, deterministic begin evidence, direct terminal outcomes, accepted-unknown recovery, causal observation outcomes, receipt-free not-dispatched recovery, and exact consumption of every action effect. Production-store Node matrices cover success, direct failure, exhausted and retryable non-application, retry, unknown applied and indeterminate observation, accepted-without-outcome recovery, absent-dispatch recovery, termination, direct compensation, and observed compensation; adversarial evidence rejects duplicate, missing, relabeled, mixed, unresolved, or contradictory members. Final reconciliation binds every component head to the exact audit, operation, saga, effect, and attempt anchor before rereading those heads through the production reader. A mutation injected during plan reconciliation is rejected by that later re-read, and fake readers or mismatched folded heads fail closed. Terminal authority is now minted only from that live registered final proof as an opaque fieldless capability backed by private weak-map identity. Its immutable state is derived from the verified operation and saga, accepted by the strict terminal model, and binds exact branch decisions, settlement outcome, and retained irreversible authorization; clones, serialization, structural fakes, and copied proofs carry no authority. Real workerd D1 evidence reloads and folds all four streams, performs the final anchor recheck, and mints capabilities for both reversible and authorized irreversible terminal sagas. This local evidence was checked against Cloudflare's current 2,000,000-byte maximum D1 row and string or BLOB sizes and transactional `D1Database.batch()` contract. It does not yet prove the compatible-reader deployment barrier or production activation, supporting-index and maximum-history performance, durable Workflow continuation, atomic capability-gated terminal-tail persistence, operation-creation/initialization/API/schema exclusivity, exact remote D1 maximum-boundary behavior, or broader remote Cloudflare qualification; each remains release-blocking.

The preceding terminal-orchestration paragraph records the Decision 87 checkpoint. Decision 88 now adds the formerly outstanding local capability-gated atomic terminal-tail proof: exact four-statement persistence, immutable replay receipts, stale-anchor and lease rejection, every rollback position, lost-response and concurrent-winner recovery, post-settlement refolding, all terminal outcome and absence-evidence modes, the 1,999,500-byte record boundary, the 256-step descriptor/515-step plan/513-transition tail boundary, and real workerd D1 execution for reversible and protocol-2 irreversible sagas. Remote maximum-boundary behavior, supporting-index and maximum-history performance, durable Workflow continuation, production activation barriers, and exclusive raw/schema authority remain release-blocking.

Decision 89 now adds the local installed-package authority boundary: the generic public operation store cannot create any saga-shaped plan, the internal factory is the sole marked creation path, mutable attempt and coordinator stores are absent from the root, and the package publishes no internal subpath. This supersedes the preceding checkpoint's statement that the operation-creation and initialization package API remained open. Exclusive deployed authority is still not proved: a future user-owned controller must exclusively own the Control D1 binding, schema installation, compatible-reader activation, and every internal mutation import, with configuration and deployment evidence that no sibling Worker or external caller has raw access.

Decision 90 now adds the additive version-5 reader-attestation schema, exact active-deployment convergence model, opaque local proof, immutable barrier receipt, and atomic production-format activation mechanism. Cloudflare's current Workers documentation defines a deployment as the active traffic assignment and permits one or two versions during a gradual deployment; the deployments API lists the latest actively serving deployment first and exposes each version ID and percentage. Nozzle conservatively normalizes those percentages to exact basis points and fails closed outside that representation. This local verifier does not authenticate its own normalized input. The controller-side Cloudflare API adapter, external attestation signature verification, freshness/race handling, and remote activation remain required before a release or any real resource activation.

Decision 91 now supplies that read-only Cloudflare deployments API adapter and exact response evidence. It supersedes only Decision 90's statement that no controller-side API adapter existed. A caller still cannot turn an API observation into a reader-barrier capability: signed attestation verification, exact bounded multi-script observation, freshness and deployment-race control, exclusive controller composition, permission validation, and remote qualification remain release-blocking.

Decision 92 now adds account-bound opaque API observations, exact bounded multi-script convergence, and standard Ed25519 attestation verification. It supersedes Decision 91's statement that signatures and bounded convergence were wholly absent. Durable signature evidence, live Worker-version artifact comparison, a second deployment read around activation, exclusive controller ownership, permission validation, post-activation enforcement, and remote qualification remain release-blocking; no public API can activate the Control format from this capability.

Decision 93 now adds live Worker-version etag observations and makes every signed artifact claim match an account-bound API proof. It supersedes Decision 92's statement that the artifact checksum remains only a trusted signed assertion. Durable signed evidence, activation-bracketing deployment reads, exclusive controller ownership, permission validation, post-activation enforcement, and remote qualification remain release-blocking; no public API can activate the Control format from this capability.

Decision 94 now adds the immutable version-6 Control schema slot and activation guard for signed barrier verification. It does not yet supersede Decision 93's durable-evidence limitation because no production store can insert or reload the canonical row; the schema intentionally leaves that row and activation empty.

Decision 95 now supplies that private store and exact local activation path. It supersedes Decision 94's production-store limitation, but not the rollout-race, deployed-authority, permission, enforcement, or remote-evidence limitations: only a future user-owned controller may compose a second exact Cloudflare observation around this mutation boundary, and no test in this checkpoint contacts or mutates a real account.

Decision 96 now requires and durably records that second observation and rejects a stale proof at authoritative D1 time. It supersedes Decision 95's single-observation production activation surface and the earlier absence of a local second-pass mechanism. It does not claim that Cloudflare deploys are locked after the second read; that guarantee depends on the still-missing exclusive controller/deployment authority and continuous post-activation enforcement.

Decision 97 now provides the sole local controller composition boundary and adds an immediate exact third observation plus an explicit compatibility assertion. It supersedes Decision 96's absence of a post-commit local check, but deployed binding and secret exclusivity, durable scheduling, operation gating, compatible rotation, permission inspection, and remote qualification are still absent.

The generic operation integrity boundary was locally revalidated on 2026-07-11 with exact safety coverage over single-snapshot plan and record reconstruction, dense undecorated checksum-bearing arrays, exact checksum-covered plan steps, nested irreversible lease and proof envelopes, verified audit predecessors and exact event chains, switching accessors, proxies, post-capture mutation, never-retry recovery, and persisted round trips. This local evidence does not replace complete-history saga verification or remote qualification.

The irreversible-authorization receipt protocol was locally revalidated on 2026-07-11 with exact safety coverage over the frozen version 1 and version 2 schema artifacts; every interrupted and two-installer version 3 upgrade boundary; the prepublication old-worker window; protocol-1 backfill and post-publication quarantine; exact body shape, size, plan, step, input, lease, acquisition, fence, and D1-time guards; immutable protocol-2 derivation; preservation and authorized retry; generic and six-statement coupled rollback; commit-response loss; restart; exact and contradictory races; provider and saga attempt admission; and workerd D1 execution. The production boundary retains the body through success, failure, crash recovery, reconciliation, and terminal classification with exact safety coverage. The complete version 3 schema artifact is frozen at 165 statements and SHA-256 `9832be0c7203fb9bd45c648486a414e7a0a61cf605bfbcd38c324a92440de27d`. The full local gate passes on supported Node 22.23.1 and Node 24.4.0 with 1,106 Node tests and exact coverage of 9,553 statements, 8,757 branches, 1,473 functions, and 8,849 lines plus 45 workerd tests. This is local structural and cryptographic evidence, not remote Cloudflare qualification, exclusive raw/schema authority, or atomic capability-gated terminal-tail persistence; those remain release-blocking.

The Decision 88 milestone passes the complete local gate on supported Node 22.23.1 and Node 24.4.0 with 1,114 Node tests and exact coverage of 9,788 statements, 8,975 branches, 1,506 functions, and 9,070 lines plus 45 workerd tests. This supersedes the earlier Decision 87 checkpoint counts and its statement that local atomic capability-gated terminal-tail persistence remained unproved. It remains local evidence, not remote Cloudflare qualification or permission to activate production resources.

The Decision 89 package-authority milestone passes the complete local gate on supported Node 22.23.1 and Node 24.4.0 with 1,116 Node tests and exact coverage of 9,796 statements, 8,983 branches, 1,509 functions, and 9,078 lines plus 45 workerd tests. This remains local package and workerd evidence, not proof of exclusive deployed Control D1 authority, production reader-barrier activation, or remote Cloudflare behavior.

The Decision 90 reader-barrier milestone passes the complete local gate on supported Node 22.23.1 and Node 24.4.0 with 1,216 Node tests and exact coverage of 10,103 statements, 9,193 branches, 1,557 functions, and 9,364 lines plus 46 workerd tests. This includes fail-closed reconciliation of contradictory, incomplete, checksum-aliasing, late, future-dated, and concurrently introduced partial barrier state, plus schema-enforced JSON-field and activation-time order. It remains normalized local and workerd evidence, not authenticated Cloudflare inventory or permission to activate a real deployment.

The Decision 91 active-deployment adapter milestone passes the complete local gate on supported Node 22.23.1 and Node 24.4.0 with 1,228 Node tests and exact coverage of 10,293 statements, 9,366 branches, 1,581 functions, and 9,531 lines plus 47 workerd tests. This is deterministic local response-contract and runtime evidence; it makes no live Cloudflare API, account-permission, freshness, signature, or deployment-race claim.

The Decision 92 signed reader-convergence milestone passes the complete local gate on supported Node 22.23.1 and Node 24.4.0 with 1,243 Node tests and exact coverage of 10,539 statements, 9,510 branches, 1,619 functions, and 9,756 lines plus 48 workerd tests. This is local cryptographic and runtime evidence over fictional account and deployment identities, not durable Control audit evidence or live Cloudflare qualification.

The Decision 93 Worker-artifact milestone passes the complete local gate on supported Node 22.23.1 and Node 24.4.0 with 1,250 Node tests and exact coverage of 10,612 statements, 9,576 branches, 1,625 functions, and 9,824 lines plus 48 workerd tests. This is local response-contract, cryptographic, and runtime evidence over fictional identities and etags, not a live Cloudflare API or activation claim.

The Decision 94 signed-evidence schema milestone passes the complete local gate on supported Node 22.23.1 and Node 24.4.0 with 1,274 Node tests and exact coverage of 10,624 statements, 9,578 branches, 1,627 functions, and 9,836 lines plus 48 workerd tests. This is additive local SQLite and workerd schema evidence with fictional activation fixtures, not a production signed-evidence write or remote D1 claim.

The Decision 95 durable signed-activation milestone passes the complete local gate on supported Node 22.23.1 and Node 24.4.0 with 1,292 Node tests and exact coverage of 10,843 statements, 9,741 branches, 1,667 functions, and 10,042 lines plus 49 workerd tests. Every identity, credential, response, key, signature, artifact, and account in this evidence is fictional or generated ephemerally in-process. This is local cryptographic, SQLite, and workerd evidence; it is not a live Cloudflare permission, deployment-race, remote D1, exclusive-controller, or real activation claim.

The Decision 96 activation-stability milestone passes the complete local gate on supported Node 22.23.1 and Node 24.4.0 with 1,298 Node tests and exact coverage of 10,939 statements, 9,840 branches, 1,685 functions, and 10,136 lines plus 49 workerd tests. The maximum local pair covers 256 fictional scripts and 512 generated active versions and persists within the one-MiB verification bound. This is deterministic local and workerd evidence only; no test uses a real token, account, Worker deployment, or remote D1 database.

The Decision 97 controller-composition milestone passes the complete local gate on supported Node 22.23.1 and Node 24.4.0 with 1,304 Node tests and exact coverage of 11,061 statements, 9,921 branches, 1,712 functions, and 10,253 lines plus 49 workerd tests. All controller tests use generated keys, fictional identities and tokens, recorded API envelopes, local SQLite, or workerd D1. The subrequest budgets are structural checks against current documented limits, not a live account-capability or cost claim.

- [Cloudflare D1 limits](https://developers.cloudflare.com/d1/platform/limits/)
- [Cloudflare D1 query JSON](https://developers.cloudflare.com/d1/sql-api/query-json/)
- [Cloudflare D1 database binding and transactional batch](https://developers.cloudflare.com/d1/worker-api/d1-database/)
...
- [Cloudflare Workers deployments API](https://developers.cloudflare.com/api/resources/workers/subresources/scripts/subresources/deployments/)
- [Cloudflare Workers version detail API](https://developers.cloudflare.com/api/resources/workers/subresources/scripts/subresources/versions/methods/get/)
