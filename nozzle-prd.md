...
74. The canonical saga cancellation and timeout admission path is one typed, six-statement atomic Control transition. Authoritative D1 time and an in-batch first-cause/deadline guard choose one immutable winner; scoped stable request identity detects changed intent; historical receipt-first replay supports exact retries, restart, and newer-fence takeover without allowing receipt-owned steps to use the internal direct-outcome primitive. Exclusive authority remains a separate release-blocking proof until legacy mutable projection and generic or raw schema bypasses are closed.
