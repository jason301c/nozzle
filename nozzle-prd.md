...
21. **Saga recovery:** prove handler-version retention; shard-local effect-receipt atomicity; duplicate and contradictory receipt rollback; stale route, schema, and partition-fence rejection; exact-primary observation on the original binding; the complete confirmed, unknown, retryable non-application, exhausted non-application, terminal failure, and compensation classification matrix across both ledgers; causal newer-fence observation acceptance and atomic observation-receipt consumption for applied, non-applied, and indeterminate outcomes; accepted-without-outcome recovery bound to the same acceptance checksum in both ledgers; protocol 1 receipt quarantine and safe revalidation during the version 2 upgrade; exact replay evidence; atomic rollback at every coupled batch statement and lost-response recovery for every terminal observation outcome; every forward and compensation unknown-outcome combination; cancellation; timeout; and process or deployment restart.
